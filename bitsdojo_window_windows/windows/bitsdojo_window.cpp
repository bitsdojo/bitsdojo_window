    ...
    LRESULT handle_nccalcsize(HWND window, WPARAM wparam, LPARAM lparam)
    {
        ...
        else
        {
            const POINT border{
                ::GetSystemMetrics(SM_CXFRAME) + ::GetSystemMetrics(SM_CXPADDEDBORDER),
                ::GetSystemMetrics(SM_CYFRAME) + ::GetSystemMetrics(SM_CXPADDEDBORDER)};
            // In Windows, when window frame (i.e. controls & border) is drawn, the space
            // of the client is actually reduced to make space for the resize border (because
            // WM_NCHHITTEST is only received inside client area).
            // In modern Windows (i.e. 10 or 11), this space actually looks transparent, thus
            // it feels like the resize border is outside client area but it is actually not,
            // instead the actual client area size is reduced.
            // The thing to note here is that the top margins are not changed (see behavior
            // of file explorer for example) & |HTTOP| is handled on the parent HWND.
